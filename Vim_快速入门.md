...
key_bindings:
  ...
  - { key: T, mods: Control, action: CreateNewWindow }    # 映射 Ctr-t 新建一个窗口(tab) 默认貌似已经绑定到 command-n 上了.
  - { key: N, mods: Control, action: SpawnNewInstance}    # 映射 Ctr-n 新建 alacritty 实例

# Make `Option` key behave as `Alt` (macOS only): OnlyLeft  OnlyRight  Both  None (default)
option_as_alt: Both
